...
const recipient_email = computed(() => page.props.recipient_email)
...
function openEditModal(contact) {
    ...
    form.date_of_birth = contact.date_of_birth
    form.email = contact.email
    form.phone_number = contact.phone_number
    form.tax_id_code = contact.tax_id_code
...
function submitUpdateForm() {
    errors.value = {}
    axios.put(route('api.contacts.update', { contact: form.id }), form)
        .then((response) => {
...
const contactIdToBeDeleted = ref(null)

function openDeleteModal(id) {
    contactIdToBeDeleted.value = id
...
function deleteContact() {
    axios.delete(route('api.contacts.delete', { contact: contactIdToBeDeleted.value }))
        .then((response) => {
...
function changeSortOrder(field, order) {
    if (field === props.sortField && order === props.sortOrder) {
        field = 'created_at'
        order = 'desc'
    }
    router.get(route('dashboard'), { page: 1, sort: field, order: order })
}
...
function openExportModal() {
    modalExportOpen.value = true
}

function closeExportModal() {
    modalExportOpen.value = false
}

function exportContacts() {
    axios.post(route('api.contacts.export'))
        .then((response) => {
            if (response && response.status === 200)
            {
                closeExportModal()
    ...
    <div class="flex flex-col">
        <div class="pb-4 overflow-x-auto ">
            <div class="flex justify-end">
                <PrimaryButton @click="openExportModal" class="mb-4">Esporta in CSV e invia mail</PrimaryButton>
            </div>
            <div class="block">
                <div class="w-full overflow-x-auto border border-gray-300 rounded-lg">
                    <table class="w-full rounded-xl">
                        <thead>
                            <tr class="bg-gray-50">
                                <th scope="col" class="flex justify-between p-5 text-sm font-semibold leading-6 text-left text-gray-900 capitalize whitespace-nowrap"> 
                                    <span>Nome</span>
                                    <div class="flex justify-between">
                                        <span><ArrowDownIcon :class="{ 'text-red-500': sortField == 'first_name' && sortOrder == 'desc' }" class="w-4 cursor-pointer" @click="changeSortOrder('first_name', 'desc')" /></span>
                                        <span><ArrowUpIcon :class="{ 'text-red-500': sortField == 'first_name' && sortOrder == 'asc' }" class="w-4 cursor-pointer" @click="changeSortOrder('first_name', 'asc')" /></span>
                                    </div>
                                </th>
                                <th scope="col" class="p-5 text-sm font-semibold leading-6 text-left text-gray-900 capitalize whitespace-nowrap"> Cognome </th>
                                <th scope="col" class="p-5 text-sm font-semibold leading-6 text-left text-gray-900 capitalize whitespace-nowrap"> Data di nascita </th>
                                <th scope="col" class="p-5 text-sm font-semibold leading-6 text-left text-gray-900 capitalize whitespace-nowrap"> Email</th>
                                <th scope="col" class="p-5 text-sm font-semibold leading-6 text-left text-gray-900 capitalize whitespace-nowrap"> Numero di telefono</th>
                                <th scope="col" class="p-5 text-sm font-semibold leading-6 text-left text-gray-900 capitalize whitespace-nowrap"> Codice Fiscale</th>
                                <th scope="col" class="p-5 text-sm font-semibold leading-6 text-left text-gray-900 capitalize whitespace-nowrap"> Azioni </th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-300 ">
                            <tr v-for="contact in contacts" class="transition-all duration-500 bg-white hover:bg-gray-50">
                                <td class="p-5 text-sm font-medium leading-6 text-gray-900 whitespace-nowrap "> {{ contact.first_name }}</td>
                                <td class="p-5 text-sm font-medium leading-6 text-gray-900 whitespace-nowrap "> {{ contact.last_name }}</td>
                                <td class="p-5 text-sm font-medium leading-6 text-gray-900 whitespace-nowrap "> {{ moment(contact.date_of_birth).format('DD/MM/YYYY') }}</td>
                                <td class="p-5 text-sm font-medium leading-6 text-gray-900 whitespace-nowrap "> {{ contact.email }}</td>
                                <td class="p-5 text-sm font-medium leading-6 text-gray-900 whitespace-nowrap"> {{ contact.phone_number }} </td>
                                <td class="p-5 text-sm font-medium leading-6 text-gray-900 whitespace-nowrap"> {{ contact.tax_id_code }} </td>
                                <td class="flex p-5 items-center gap-0.5">
                                    <button v-if="canUpdate" @click="openEditModal(contact)" class="flex p-2 transition-all duration-500 bg-white rounded-full group hover:bg-indigo-600 item-center">
                                    ...
                                    <button v-if="canDelete" @click="openDeleteModal(contact.id)" class="flex p-2 transition-all duration-500 bg-white rounded-full group hover:bg-red-600 item-center">
                ...
                <Pagination class="mt-4" :pagination="meta" />
            </div>
        </div>
    </div>
    <Teleport to="body">
        <Modal :show="modalEditOpen" @close="closeEditModal">
            <div class="p-4">
                <h2 class="py-2 font-bold border-b-2">Modifica contatto</h2>
                <div class="grid grid-cols-2 gap-4">
                    <div class="py-2">
                        <InputLabel class="mb-2" value="Nome"></InputLabel>
                        <TextInput class="w-full" v-model="form.first_name" />
                        <template v-if="errors.first_name">
                            <InputError v-for="error in errors.first_name" :message="error" />
                        </template>
                    </div>
                    <div class="py-2">
                        <InputLabel class="mb-2" value="Cognome" required></InputLabel>
                        <TextInput class="w-full" v-model="form.last_name" />
                        <template v-if="errors.last_name">
                            <InputError v-for="error in errors.last_name" :message="error" />
                        </template>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div class="py-2">
                        ...
                        <TextInput type="date" class="w-full" v-model="form.date_of_birth" />
                    </div>
                    <div class="py-2">
                        <InputLabel class="mb-2" value="Email"></InputLabel>
                        <TextInput class="w-full" v-model="form.email" />
                        <template v-if="errors.email">
                            <InputError v-for="error in errors.email" :message="error" />
                        </template>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div class="py-2">
                        ...
                        <TextInput class="w-full" v-model="form.phone_number" />
                        <template v-if="errors.phone_number">
                            <InputError v-for="error in errors.phone_number" :message="error" />
                        </template>
                    </div>
                    <div class="py-2">
                        <InputLabel class="mb-2" value="Codice Fiscale"></InputLabel>
                        <TextInput class="w-full" v-model="form.tax_id_code" />
                        <template v-if="errors.tax_id_code">
                            <InputError v-for="error in errors.tax_id_code" :message="error" />
                        </template>
                    </div>
                </div>
                <div class="flex justify-end py-2">
                    <PrimaryButton class="mr-4" @click="submitUpdateForm">Salva</PrimaryButton>
                    <SecondaryButton @click="closeEditModal">Annulla</SecondaryButton>
                </div>
            </div>
        </Modal>
        <ConfirmationModal :show="modalDeleteOpen" @close="closeDeleteModal">
            <template #title>
                <h2 class="py-2 font-bold">Elimina contatto</h2>
            </template>
            <template #content>
                <p class="py-2">Sei sicuro di voler eliminare il contatto?</p>
            </template>
            <template #footer>
                <div class="flex justify-end py-2">
                    <PrimaryButton class="mr-4" @click="deleteContact">Conferma</PrimaryButton>
                    <SecondaryButton @click="closeDeleteModal">Annulla</SecondaryButton>
                </div>
            </template>
        </ConfirmationModal>

        <ConfirmationModal :show="modalExportOpen" @close="closeExportModal">
            <template #title>
                <h2 class="py-2 font-bold">Esporta contatti</h2>
            </template>
            <template #content>
                <p class="py-2">I contatti verranno esportati e inviati all'indirizzo <b>{{ recipient_email }}</b>.</p>
                <p>Procedere?</p>
            </template>
            <template #footer>
                <div class="flex justify-end py-2">
                    <PrimaryButton class="mr-4" @click="exportContacts">Conferma</PrimaryButton>
